...
def load_schema() -> Dict:
    ...
    schema_path = Path('vendor-schema.yml')
    if not schema_path.exists():
        print(f"{Colors.RED}Error: vendor-schema.yml not found{Colors.NC}")
        sys.exit(1)
...
def load_taxonomy() -> Dict:
    """Load the taxonomy."""
    taxonomy_path = Path('taxonomy.yml')
    if not taxonomy_path.exists():
        print(f"{Colors.YELLOW}Warning: taxonomy.yml not found, skipping taxonomy validation{Colors.NC}")
